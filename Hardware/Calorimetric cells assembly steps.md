Assembling the calorimetric cells is quite straight forward once you have all the components: Peltier cells, aluminium bloc (heat sink), thermal conductive paste, 3D printed channels separator and PVC tube.

First, place the Peltier cells onto the aluminium block with thermal conductive paste at the interface between the aluminium and peltier cells.
![peltier cells assembly](images/assembly_peltier_cells.png)

Then, place the channels separator; you can hot glue it to the aluminium block to ensure it won't move.
![separator placement](images/assembly_channels_separator.png)

After that, place the PVC tube and pass the cables out from the top of the PVC tube.
![PVC tube](images/assembly_pvc_tube.png)

Now the cell needs to be water-tight from under; for that, place tightning collars around the PVC tube to firmly clamp it to the block; you can then tape the PVC tube to the aluminium block to create a temporary water-tight-ish assembly. Then poor liquide silicone in the small gap between the tube and block; when it hardens it will be fully water-tight.
![liquid silicone](images/assembly_liquid_silicone.png)

You can now spray isolating foam all around the PVC tube and channels separator. Choose a foam adapted for moist environments.
![spray foam](images/assembly_foam.png)

Once the foam has hardened, you can cut the exceedent, revealing your calorimetric channels.
![cut foam](images/assembly_cut_foam.png)

Do the same procedure for the channel lids.
![lids](images/assembly_lids.png)

Enjoy your DIY-made calorimetric cell! Congrats!
![cells](images/assembly_calorimetric_cell.png) 
![cells_2](images/assembly_calorimetric_cell_2.png)
